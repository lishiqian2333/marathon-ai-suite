...
function PptDemo(){
  ...
  useEffect(()=>{if(!busy)return;const timer=window.setInterval(()=>setProgress(p=>{const next=Math.min(100,p+4);if(next===100){if(stage==="outlining"){setStage("outline");setMessages(m=>[...m,{role:"ai",text:"提纲已完成，共规划 12 页，重点覆盖赛事价值、鹏飞合作价值、冠名权益、AI 文创与合作建议。",card:"已生成 12 页大屏汇报提纲"}])}else{setStage("preview");setMessages(m=>[...m,{role:"ai",text:"PPT 已生成并完成大屏版式检查。你可以继续让我定位页面、精简结构或生成讲解建议。",card:"PPT 已生成 · 12 页"}])}}return next}),90);return()=>window.clearInterval(timer)},[busy,stage]);
